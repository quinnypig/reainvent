...
if (!database) {
  execFileSync("npx", ["wrangler", "d1", "create", name, "--no-update-config"], { stdio: "inherit" });
  database = findDatabase();
}
if (!database?.uuid || !/^[a-f0-9-]{36}$/i.test(database.uuid)) throw new Error("Could not resolve the account database ID.");
if (!process.env.GITHUB_ENV) throw new Error("This bootstrap expects a GitHub Actions deployment environment.");
