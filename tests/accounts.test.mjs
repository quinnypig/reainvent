...
function request(path, body, cookie = "", origin = "https://example.com") {
  return new Request(`https://example.com/api/account${path}`, {
...
test("signup, session restore, logout, login, account deletion", async () => {
  ...
  assert.equal(
    (await accountRequest(request("/signup", credentials), db)).status,
    409,
  );
  assert.equal(
    (
  ...
  assert.equal(
    (await (await accountRequest(request("", null, cookie), db)).json()).user,
    null,
  );
...
test("rejects cross-origin writes, weak passwords, unavailable storage and expired sessions", async () => {
  ...
  assert.equal(
    (
      await accountRequest(
        request("/signup", credentials, "", "https://attacker.example"),
        db,
      )
    ).status,
    403,
  ...
  assert.equal(
    (
      await accountRequest(
        request("/signup", { ...credentials, password: "short" }),
        db,
      )
    ).status,
    400,
  ...
  assert.equal(
    (await accountRequest(request("/signup", credentials), undefined)).status,
    503,
  );
  const signup = await accountRequest(request("/signup", credentials), db);
  ...
  assert.equal(
    (
      await (
        await accountRequest(
          request("", null, signup.headers.get("set-cookie")),
          db,
        )
      ).json()
    ).user,
