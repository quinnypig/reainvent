...
export async function accountRequest(request, db) {
  ...
  if (!db)
    return json(
      { error: "Accounts are not available yet. Please try again later." },
      503,
    );
  if (
    request.method !== "GET" &&
  ...
  if (
    (!signup && pathname !== "/api/account/login") ||
    request.method !== "POST"
  )
  ...
  if (
    ...
    return json(
      {
        error:
          "Use a username of 3–24 letters, numbers, or underscores and a password of 12–128 characters.",
      },
      400,
    );
  ...
  for (const scope of [ip, `account:${username}`]) {
    ...
    const limit = await db
      ...
      .bind(key, now + period)
      .first();
    if (limit.attempts > (signup ? 10 : 20))
      return json(
        { error: "Too many attempts. Please try again later." },
        429,
        { "retry-after": String(period - (now % period)) },
      );
  }
  ...
  if (signup) {
    ...
    try {
      await db
        ...
        .bind(account.id, username, passwordHash, now)
        .run();
    } catch (error) {
      if (String(error).includes("UNIQUE"))
        return json({ error: "That username is unavailable." }, 409);
      throw error;
    }
  } else {
  ...
  await db.batch([
    ...
    db
      ...
      .bind(digest(freshToken), account.id, now + WEEK),
  ]);
  return json({ user: { username } }, signup ? 201 : 200, {
    "set-cookie": cookie(request, freshToken),
  });
}
